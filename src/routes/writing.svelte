...
<script>
	...
	// $: if (filter) {
	$: console.log(posts.map((post) => post.type));

	// $: posts = posts.filter((post) => post.type === selectTag);
	// }
...
<div class="flex flex-row ">
	<div class="flex-none w-40 sticky top-4 self-start">
		{#each tags as tag}
			<button class="hover:font-bold block" on:click={() => (selectTag = tag)}>
				{tag}
			</button>
		{/each}
	</div>
	...
	<div class="grow">
		...
		>
			<button
				class="fixed sm:relative sm:py-2 sm:bg-white"
			...
			{#if searching}
				<input
					...
					name="searchTerm"
					id="searchTerm"
					placeholder="Search by title, artist, publisher..."
					autofocus
				/>
			{/if}
		</div>
